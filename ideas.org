...
Recordemos que un conjunto $A$ es un subconjunto analítico complejo de la variedad analítica $M$ si es cerrado y todo punto $x\in A$ posée una vecindad $U_x$ tal que $A\cap U_x=V(f_1,\dots,f_k)$ donde $V(F)$ denota el conjunto de ceros de la familia de funciones en $F$ y $f_j:U_x\rightarrow\co$. Un conjunto analítico en general no tiene una estructura de variedad diferencial. Por lo que es necesario defeinir un /punto ordinario/ de $A$ como un punto $x\in A$ tal que en una carta $(U_x,\varphi)$ de $x$, $U_x\cap A$ es biholomorfa a una vecindad de un subespacio lineal $L^{k}\subset\con$ por medio de una restricción de $\varphi$, la dimensión compleja de $L^{k}$, $k$ es la dimensión de $A$ en $x$, denotaremos por $A'$ al conjunto de puntos ordinarios. Los puntos ordianarios forman un subconjunto analítico de $A$ de dimensión $k'\leq k$. Si $A$ es de dimensión $k$ en todos sus puntos, diremos que $A$ es de dimension pura $k$.
...
*** Corrientes positivas y corrientes positivas cerradas.
#+BEGIN_QUOTE
El operador $t_0(\phi)$ es una corriente en el sentido de De Rham, también conocida como "/forma diferenecial generalizada/" o forma diferencial con coeficientes que son distribuciones. El espacio es describe como el espacio de las expresiones del tipo
\[
...
\]
El siguiente resulltado permite simplificar la demostraci
\begin{prop}
Sea $A$ un conjunto analitico de dimensión pura $k$ que contiene el origen
\end{prop}
