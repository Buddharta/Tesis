...
\end{equation}
If $X$ is a purely $k$-dimensional analytic subset of $\con$, the singular locus of $X$, $\Sigma(X)$ is the set of points where the defining functions of $X$, say $X\cap U=V(f_{1},\dots,f_{n-k})$ have derivatives that span a space of dimension \emph{less than $k$}, meaning that the matrix defined by the partial derivatives
\[
  M=\left(\dfrac{\partial f_{j}}{\partial z_{i}}\right)_{ij}
\]
has a rank \emph{less than $k$}.Therefore $M=X\setminus\Sigma(X)$ is a complex manifold and $\Sigma(X)$ is an analytic subset of $X$ of lesser dimension, defined as the null set of the square submatrices of $M$ and so its volume is negligible, i.e.
\[
    \textrm{Vol}_{2k}(X)= \frac{1}{k!}\int_{X} \omega^k= \frac{1}{k!}\int_{X\setminus\Sigma(X)} \omega^k=
...
A $p$-dimensional calibrated submanifold of a manifold $M$ with calibration $\varphi$ is an oriented submanifold $\Sigma$ such that the calibration restricted to the tangent bundle of $\Sigma$, equals the induced volume form of the submanifold $\varphi|_{T\Sigma}=\textrm{Vol}_{\Sigma}$. Equivalently $T\Sigma\subset G(\varphi)$
\end{definition}
