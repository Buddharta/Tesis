...
Now, given an analytic set $X$, the analytic set of regular points of $X$ is $X_{reg}:=X\setminus \sig(X)\subset U':=U\setminus X_{reg}$, where $X_{reg}$ is the set of regular points of $X$ so we will say that $U'$ is the open subset of regular points of $U$, then the problem of finding $\tau(\phi)$ is the problem of extending the operator
\[
    \tau_0(\phi)=\int_{X_{reg}}\phi
\]
from the space $\om^{(k,k)}_c(U')$ (where it is well-defined in the classical sense) to $\om_{(k,k)}(U)$. This is a way to reformulate in terms of \emph{integration currents} the problem that Bishop tackled and solved.
\subsubsection{Positive Currents and Closed Positive Currents}
The operator $\tau_0(\phi)$ is a current in the sense of de Rham, also known as a \emph{generalized differential form} or differential form with coefficients that are distributions (linear functionals). The space is described as the space of expressions of the type
\[
    \tau=\sum_{i_1,\dots i_{n-p},j_1,\dots j_{n-p}}\tau_{i_1\dots i_{n-k}j_1\dots j_{n-k}} dz_{i_1}\wedge\dots\wedge dz_{i_{n-k}}\wedge d\overline{z}_{j_1}\wedge\dots\wedge d\overline{z}_{j_{n-k}}.
\]
